...
public class POW {
     ...
     */
    private void setUP() {
        int screenWidth = Game.gameWidth;
        switch(screenWidth){
            case 300:
                pow_w = 25;
                pow_h = 25;
    ...
    */
    public static void render(Graphics2D g2d) {
        cannonPOW.render(g2d);
        shieldPOW.render(g2d);
        speedPOW.render(g2d);
        extraLifePOW.render(g2d);
    }
    ...
    private static void checkCollisions() {
        ...
        for(int c = 0; c < cannonPOWz.size(); c++){
            if(cannonPOWz.get(c).isDead == false){
                ...
                if(cannonPOWz.get(c).getBounds().intersects(Game.paddle.P1Rect)){
        ...
        for(int c = 0; c < shieldPOWz.size(); c++){
            if(shieldPOWz.get(c).isDead == false){
                ...
                if(shieldPOWz.get(c).getBounds().intersects(Game.paddle.P1Rect)){
        ...
        for(int c = 0; c < speedPOWz.size(); c++){
            if(speedPOWz.get(c).isDead == false){
                ...
                if(speedPOWz.get(c).getBounds().intersects(Game.paddle.P1Rect)){
        ...
        for(int c = 0; c < extraLifePOWz.size(); c++){
            if(extraLifePOWz.get(c).isDead == false){
                ...
                if(extraLifePOWz.get(c).getBounds().intersects(Game.paddle.P1Rect)){
                    
                    //message POW hit  POWHit(int id)
                    //extraLifePOWz.get(c).POWHit(c);
                    POWHit(extraLifePOWz.get(c));
                    Game.paddle.extraLife();
