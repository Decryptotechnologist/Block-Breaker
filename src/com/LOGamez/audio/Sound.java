...
public class Sound implements LineListener {
    ...
    */    
    public Sound(String filename, double vol){
        ...
        Clip oldSnd = BlockBreaker.sndMap.get(filename);
        if(oldSnd != null){
            this.aClip = oldSnd;
        } else {
            try{
                System.out.println("Loading Sound: "+BlockBreaker.soundCount+": " + audioFileName);
                ...
                setVolume(vol);
                BlockBreaker.sndMap.put(filename, aClip);
                BlockBreaker.soundCount++;
            } catch(Exception e){
            }
        }
    }
    ...
    */    
    public Sound(String filename){
        ...
        AudioClip oldAudio = BlockBreaker.audioMap.get(filename);
        if(oldAudio != null){
            this.clip = oldAudio;
        } else {
            try{
                System.out.println("Loading Audio: "+BlockBreaker.audioCount+": "+"/snd"+ filename);
                clip = Applet.newAudioClip(Sound.class.getResource("/snd"+filename));
                BlockBreaker.audioMap.put(filename, clip);
                BlockBreaker.audioCount++;
            } catch (Exception e) {
            }
        }
    }
    ...
    protected void finalize() throws Throwable {
        if(!fileName.isEmpty()){ 
            BlockBreaker.sndMap.remove(fileName);
            System.out.println("Removing Sound from memory: " + fileName);
        }
        if(!audioFileName.isEmpty()){ 
            BlockBreaker.audioMap.remove(audioFileName);
